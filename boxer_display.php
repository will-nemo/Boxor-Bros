...
<!-- THIS IS THE CODE THAT CHANGES FOR EACH PAGE-->
    ...
    <div class="w3-sidebar w3-light-grey w3-bar-block" style="padding-bottom:25%; width:25%;">
        <p>Type in the Max Price of the Product you want</p>
    
        <?php
         echo'<form method="POST" action="boxer_display.php">
         <input type="text" name="priced">';
        
        echo '<button class="w3-margin w3-blue w3-btn w3-center" type="submit" name="psubmit" >Search</button></form>';  
   ...
   if (isset($_GET['priced'])) {
       $pricess = $_GET['priced'];
      
        $search = "SELECT * FROM merch HAVING price < '$pricess'";
  }else{ 
  ...
  if (isset($_GET['query'])) {
    ...
    if(isset($_GET['search'])){
        $merchss = $_GET['search'];
        $search = "SELECT * FROM merch WHERE name LIKE '%". $merchss ."%'";
    }else{
        ...
        if($records->num_rows > 0){ 
            ...
            echo '<div class="w3-row-padding " style="padding-left:25%; padding-right:10px;">';
            ...
            while($row = $records->fetch_assoc()){
                ...
                echo '<div class="w3-card-4 w3-third w3-padding-large w3-hover-opacity">
                    <a href="item_display.php?varname=' .$name. '">';

                echo'<img src="' .$row["IMG"].'"  style="width:300px; height:300px;"></a>
                    <p>'.$row["name"].'</p>
                    <p>&#36;'.$row["price"].'</p>
                </div>';
            }
